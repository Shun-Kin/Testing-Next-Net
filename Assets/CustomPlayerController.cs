...
public class CustomPlayerController : NetworkBehaviour
{
        ...
        if (isLocalPlayer && !isDead)
        {
            ...
            if (Physics.CheckSphere(groundCheck.position, groundDistance, groundMask))
            {
                if (!grounded)
                {
                    ...
                    float minVelocity = _rb.velocity.y < lastVelocityY ? _rb.velocity.y : lastVelocityY;    // Скорость из предыдущего вызова FixedUpdate может быть меньше
    ...
    private void Move()
    {
        ...
        _rb.AddForce(velocityChange, ForceMode.VelocityChange);       // ForceMode.VelocityChange - мгновенно изменить скорость
    }
    ...
    private async UniTaskVoid RespawnInAsync(int millisecondsDelay, CancellationToken cancellationToken)
    {
        ...
        GameManager.ReplacePlayer(connectionToClient);    // Создание нового объекта игрока
    }
